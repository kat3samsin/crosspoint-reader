...
size_t TextBlock::arenaSize(const uint16_t wordCount, const bool hasFocus, const uint16_t textBytes) {
  // Layout documented in TextBlock.h: aligned integer arrays, then byte arrays and text.
  size_t size =
...
void TextBlock::bindArenaPointers() {
  ...
  sourceOrdinalArr = reinterpret_cast<const uint32_t*>(base);
...
TextBlock::TextBlock(const std::vector<std::string>& words, const std::vector<int16_t>& wordXpos,
  ...
  if (words.size() != wordXpos.size() || words.size() != wordStyles.size() ||
      words.size() != wordSourceOrdinals.size() || words.size() > 10000 ||
      (hasFocus && (words.size() != focusBoundary.size() || words.size() != focusSuffixX.size()))) {
    LOG_ERR("TXB",
            "Construction failed: size mismatch (words=%u, xpos=%u, styles=%u, source=%u, boundary=%u, suffixX=%u)",
            static_cast<uint32_t>(words.size()), static_cast<uint32_t>(wordXpos.size()),
            static_cast<uint32_t>(wordStyles.size()), static_cast<uint32_t>(wordSourceOrdinals.size()),
  ...
  auto* sourceOrdinals = const_cast<uint32_t*>(sourceOrdinalArr);
  ...
  for (uint16_t i = 0; i < numWords; i++) {
    sourceOrdinals[i] = wordSourceOrdinals[i];
    textOff[i] = off;
    xpos[i] = wordXpos[i];
